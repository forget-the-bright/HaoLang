...
- 当前版本：**v0.55.5**（循环 spill 池 + while/for 提升 + remset 仅 minor + 精确根完备 + 皮带成对摘根 + 诚实双轨 + 混合屏障 + mark worker；详文 [`docs/IR与GC契约.md`](docs/IR与GC契约.md)）
- 测试基线：`test/suite` **1011** 行 stdout，退出码 0；反向 `script/win/negcheck.ps1` 28/28；`hao version` = 0.55.5
...
- GC 是 **v0.55.5**：混合屏障 + 循环 spill 池 + while/for 提升 + remset **仅服务 minor** + Hao shadow 精确根 + C 皮带成对摘根 + mark worker；**诚实双轨**。详文 [`docs/IR与GC契约.md`](docs/IR与GC契约.md)。
...
| 状态 | 内容 |
|------|------|
| ✅ 已完成 | **v0.55.5** 循环 spill 池 + while/for 提升 + remset 仅 minor + 精确根完备 + 皮带成对 + 诚实双轨；反向 28/28 |
