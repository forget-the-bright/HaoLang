...
- [x] 薄 `llvm.dbg.declare`：`-g` 下函数参数与直接 `varDecl` 可见地址/槽名（v0.55.37）；**catch 绑定 / for 循环变量**亦 declare（v0.55.38）；Except `setjmp`/`switch` 薄挂 `!dbg`；select 调度 `switch` 附 `!dbg`（v0.55.39）；**非**完整 Hao 类型美化 / 非「每条 IR」。
- [x] 薄 `llvm.dbg.value`：`-g` 下带初值的直接局部声明发射 ≥1 条（v0.55.41）；简单局部赋值（v0.55.42）；复合赋值与实例/静态字段 `=`（v0.55.43）；数组下标 `=`/复合写回（v0.55.44）；字段/静态**复合**写回（v0.55.45）；for 循环变量每轮迭代（v0.55.46）；catch 绑定（v0.55.47）；**非**完整 Hao 类型视图 / **非**全覆盖。
