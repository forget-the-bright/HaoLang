...
**软 STW（v0.60.4 握手 + v0.71 所有权 + v0.72 fail-fast + v0.74 LAT-2 + v0.75～0.76 HTTP-LAT/JSON）**：跨轮保持 `stw_request`。**v0.72 删除 HOT_GRACE**；根/终止预算 **16/12ms**、**2×6ms**、stall **4ms**。**v0.74**：collect 禁单锁贯穿 drain；`gc_stats_publish` 双缓冲；`hao_gc_stats` 读快照；`lastCollectHoldMs`。**v0.75**：分段 ms；`lastStatsSafepointMs`；默认 keep-alive（`HAO_HTTP_API_SHORT=1` 短连）；HttpClient 压测。**v0.75.1**：默认 `/api/gc` **全量**。**v0.75.2**：`JsonBeanInfo`+`field_get_at`；删手写 `toJson`。**v0.76**：`lang.StringBuilder`；JsonBuf/ByteBuf 真缓冲；IRGen `$jsonWrite`+meta；**jsonP95≤8**。门禁 `stw_handshake_gate` / `gc_stw_ownership_gate` / `gc_stw_budget_gate` / `gc_lat2_source_gate` / **`gc_http_lat_source_gate`** / **`gc_api_latency_gate`** / **`stringbuilder_gate`**。abort≈incomplete 且 collect 升 = **协作税，禁止误判 UAF**。Win 无信号抢占 → incomplete 合法。**SuspendThread 已删除**。
