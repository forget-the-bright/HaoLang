...
echo "========================================"
echo "套件总行数: $total  (基线 1052) | 退出码: $code"
[[ $code -eq 0 ]] || exit 1
# P3 双门禁：行数必须等于基线（曾出现增量缓存 suite 少行误报 1007）
if [[ "$total" -ne 1052 ]]; then
  echo "FAIL 套件行数 $total != 1052（请 --rebuild-all 或查丢打印）"
  exit 1
fi
# 基线：v0.79 复核 1052（与 v0.76 同）；v0.78 曾误记 1050；v0.60.2 → 1049
