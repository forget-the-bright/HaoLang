...
echo "========================================"
echo "套件总行数: $total  (基线 1032) | 退出码: $code"
[[ $code -eq 0 ]] || exit 1
# P3 双门禁：行数必须等于基线（曾出现增量缓存 suite 少行误报 1007）
if [[ "$total" -ne 1032 ]]; then
  echo "FAIL 套件行数 $total != 1032（请 --rebuild-all 或查丢打印）"
  exit 1
fi
# 基线：v0.59.0 langbox 隐式装箱 +3 → 1032；v0.58.0 / v0.57.x 曾 1029；v0.56.0 曾 1027；v0.55.60 曾 1029
